...
if not st.session_state.sudah_masuk:
    ...
    with col_btn:
        if st.button("Masuk ke Core 🔮", key="enter_cyber_btn", use_container_width=True):
            ...
            st.session_state.splash_time = time.time()  # Catat waktu mulai transisi splash
            st.rerun()
...
# 💬 HALAMAN 2: INTERFACE CHAT CORE UTAMA (EFEK TRANSISI SPLASH SCREEN SINKRON)
else:
    # 🎬 FASE 2A: JIKA SEDANG TRANSISI SPLASH (HANYA MUNCUL BUMI & NAMA SEPERTI AWAL VIDEO)
    if not st.session_state.splash_done:
        ...
        # Jeda splash screen selama 2.2 detik biar transisinya berasa pas
        time.sleep(2.2)
        st.session_state.splash_done = True
        st.rerun()

    # 🎬 FASE 2B: SETELAH SPLASH SCREEN SELESAI, TAMPILKAN SELURUH ISI CHAT DAN LAYOUT KAPSUL
    else:
        st.markdown('<div class="content-fade-in">', unsafe_allow_html=True)
        ...
        st.markdown('<p class="oxy-sub-title" style="margin-bottom:15px !important; font-size:0.9rem;">Saya asisten AI yang siap membantu Anda. Silakan ketik pertanyaan atau unggah gambar untuk dianalisis.</p>', unsafe_allow_html=True)

        # KAPSUL LAYOUT SEPERTI DI REFERENSI VIDEO
        st.html("""
        ...
        if user_input := st.chat_input("Ketik pesan..."):
            ...
            if len(st.session_state.messages) == 0:
                st.session_state.messages.append({
                    "role": "system", 
                    "content": "You are oXy AI, a highly advanced artificial intelligence developed by Zayn. Always assist users professionally."
                })
